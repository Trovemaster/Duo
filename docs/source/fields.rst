...
From 2023, for the electronic states can be labelled using strings of characters, e.g.
::
...
``dipole-x`` is related to the Cartesian-representation (see below).
...
Internally, Duo represents electronic basis functions in Hund’s case (a), where the electronic function
:math:`|\Lambda\rangle` is an eigenfunction of :math:`\hat{L}_z`,

.. math::

   \hat{L}_z |\Lambda\rangle = \hbar \Lambda |\Lambda\rangle .

In this *\Lambda-representation* (also referred to as the **tensorial** representation), the :math:`z` component is diagonal
and carries the signed value :math:`\Lambda`.  Duo therefore stores and manipulates coupling matrix elements and transition
moments in this tensorial form. For example, the electronic angular momentum ladder operators are

.. math::

   \hat{L}_{\pm} = \hat{L}_x \pm i \hat{L}_y ,

and the spherical (tensorial) components of the body-fixed dipole moment are

.. math::

   \hat{\mu}_{\pm} = \mp \frac{1}{\sqrt{2}}\left(\hat{\mu}_x \pm i \hat{\mu}_y\right),
...
:math:`|{\rm state},\Lambda,S,\Sigma\rangle`, all coupling matrix elements in the \Lambda-representation can be made real.
Note that the underlying electronic functions in the \Lambda-representation are complex (they contain factors of the form
:math:`e^{i\Lambda\phi}`, where :math:`\phi` is the azimuthal angle about the molecular :math:`z` axis).

For that reason, the \Lambda-representation is the native representation for Duo. The corresponding input objects assume
...
Cartesian representation. For many multi-component properties, only one Cartesian component is required (a reference
component), since all other non-zero matrix elements can be generated by symmetry/selection rules. Therefore, for
Cartesian input Duo typically requires **only the x component**.
...
.. math::
   :label: eq-lambda-transf

   |\pm\Lambda\rangle = C_{1}^{\pm|\Lambda|}\,|1\rangle + C_{2}^{\pm|\Lambda|}\,|2\rangle ,

where :math:`|1\rangle` and :math:`|2\rangle` denote the relevant Cartesian components (e.g. :math:`x,y` or other real
combinations such as :math:`xy,\ldots`) corresponding to Abelian point-group symmetry pairs (e.g. :math:`A_1,B_1` or
:math:`A_2,B_2`). The coefficients :math:`C_{\alpha}^{\Lambda}` are elements of the unitary transformation from the
Cartesian basis to the \Lambda-representation.

For non-diagonal spin–orbit couplings with :math:`|\Delta\Lambda|=1` reported by Molpro, the relevant Cartesian reference matrix element is an :math:`x` component, e.g. :math:`\langle 1.1 | {\rm LSX} | 1.3 \rangle` (use ``spin-orbit-x``). In the \Lambda-representation the corresponding matrix element acquires a factor of :math:`\sqrt{2}`.


.. note::
   Do not mix representations within the same object. If the electronic wavefunctions and property are taken from a
   Cartesian quantum-chemistry calculation, use the corresponding ``*-x`` objects consistently (e.g. ``spin-orbit-x``,
   ``dipole-x``).

.. note::
   For perpendicular dipole transitions with :math:`|\Delta\Lambda|=1`, the Cartesian :math:`x` component (``dipole-x``),
   e.g. :math:`\langle 1.1 | {\rm DMX} | 1.2 \rangle`, has the **same numerical value** as the tensorial body-fixed
   component used internally by Duo. The :math:`\sqrt{2}` factor from the basis transformation is cancelled by the
   :math:`1/\sqrt{2}` in the definition of :math:`\mu_{\pm}` above. Therefore, **do not apply any manual**
   :math:`\sqrt{2}` conversion for dipoles; ``dipole`` and ``dipole-x`` accept the same numerical values and Duo handles
   the transformation internally.
