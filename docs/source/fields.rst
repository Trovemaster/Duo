...
.. note::
   For non-diagonal spin–orbit couplings with :math:`|\Delta\Lambda|=1` reported by Molpro, the relevant Cartesian
   reference matrix element is an :math:`x` component, e.g. :math:`\langle 1.1 | {\rm LSX} | 1.3 \rangle` (use
   ``spin-orbit-x``). In the \Lambda-representation the corresponding matrix element acquires a factor of :math:`\sqrt{2}`.
