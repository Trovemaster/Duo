module diatom_module
  ...
  ! 7. Add a new case to all cases where the fields appear: at the end of the input subroutine,
  ...
  !
  !
  ! Lorenzo Lodi: it is necessary to repeat the character length
  ! specification in the array contructor for Fortran2003 conformance
  character(len=wl), parameter :: &
    CLASSNAMES(1:Nobjects) = (/character(len=wl):: &
    "POTEN", "SPINORBIT", "L2", "L+", "SPIN-SPIN", "SPIN-SPIN-O", &
    "BOBROT", "SPIN-ROT", "DIABATIC", "LAMBDAOPQ", "LAMBDAP2Q", &
    "LAMBDAQ", "ABINITIO", "BROT", "DIPOLE", "QUADRUPOLE"/)
  !
  ! Lorenzo Lodi
  ...
  type(fieldT), pointer      :: poten(:), spinorbit(:), l2(:), lxly(:), &
                                abinitio(:), dipoletm(:) => null(), &
                                spinspin(:), spinspino(:), bobrot(:), &
                                spinrot(:), diabatic(:), lambdaopq(:), &
                                lambdap2q(:), lambdaq(:), &
                                quadrupoletm(:) => null()
  ...
  !type(symmetryT)           :: sym
  !
  integer(ik)   :: nestates, Nspinorbits, Ndipoles, Nlxly, Nl2, Nabi, &
                   Ntotalfields = 0, Nss, Nsso, Nbobrot, Nsr, Ndiabatic, &
  ...
  !
  public ReadInput, poten, spinorbit, l2, lxly, abinitio, brot, &
         map_fields_onto_grid, fitting, jmin, jmax, vmax, fieldmap, &
         Intensity, eigen, basis, Ndipoles, dipoletm, linkT, three_j &
         Nquadrupoles, quadrupoletm
...
contains
  ...
  subroutine ReadInput
    ...
    integer(ik)  :: ipot = 0, iso = 0, ncouples = 0, il2 = 0, ilxly = 0, &
                    iabi = 0, idip = 0, iss = 0, isso = 0, ibobrot = 0, &
                    isr = 0, idiab = 0, &
                    iquad = 0
    integer(ik)  :: Nparam, alloc, iparam, i, j, iobs, i_t, iref, jref, &
                    istate, jstate, istate_, jstate_, item_, ibraket, &
                    iabi_, iterm, iobj
    ...
    do
      ...
      case ("NSTATES", "NESTATES")
        ...
        !
        allocate (poten(nestates), spinorbit(ncouples), l2(ncouples), lxly(ncouples), &
                  spinspin(nestates), spinspino(nestates), bobrot(nestates), &
                  spinrot(nestates), job%vibmax(nestates), job%vibenermax(nestates), &
                  diabatic(ncouples), lambdaopq(nestates), lambdap2q(nestates), &
                  lambdaq(nestates), quadrupoletm(nestates), stat=alloc)
        !
      ...
      case ("CHECK_POINT", "CHECKPOINT", "CHECKPOINTS")
        ...
        do while (trim(w) /= "" .and. trim(w) /= "END")
          ...
          case ('VECTOR-FILENAME', 'VECTOR', 'FILENAME')
            ...
            !
          case ('TM', 'DIPOLE') !!! do we need an equivalent quadrupole here?
            !
      ...
      case ("FITTING")
        ...
        !
      case ("SPIN-ORBIT", "SPIN-ORBIT-X", "POTEN", "POTENTIAL", "L2", "L**2", &
            "LXLY", "LYLX", "ABINITIO", "LPLUS", "L+", "L_+", "LX", "DIPOLE", &
            "TM", "DIPOLE-MOMENT", "DIPOLE-X", "SPIN-SPIN", "SPIN-SPIN-O", &
            "BOBROT", "BOB-ROT", "SPIN-ROT", "DIABATIC", "DIABAT", &
            "LAMBDA-OPQ", "LAMBDA-P2Q", "LAMBDA-Q", "LAMBDAOPQ", "LAMBDAP2Q", &
            "LAMBDAQ", "QUADRUPOLE", "QUADRUPOLE-MOMENT")
        !
        ibraket = 0
        !
        ! initializing units
        ...
        case ("LAMBDA-Q", "LAMBDAQ")
          ...
          if (action%fitting) call report(trim(field%class)//" cannot appear after FITTING", .true.)
          !
        case ("QUADRUPOLE", "QM", "QUADRUPOLE-MOMENT")
          !
          if (iquad == 0)
          ...
          loop_quad: do istate = 1, Nestates
            do jstate = 1, Nestates
              !
              if (      iref == poten(istate)%iref &
                  .and. jref == poten(jstate)%iref) then
                include_state = .true.
                istate_ = istate
                jstate_ = jstate
                exit loop_quad
          ...
          if (iquad > ncouples) then
            write (out, "(2a,i4,a,i6)") trim(w), &
              ": Number of quadrupoles = ", iquad, &
              " exceeds the maximal allowed value", ncouples
            call report("Too many couplings given in the input for"&
                        //trim(w), .true.)
          endif
          !
        ...
        case ("ABINITIO")
          ...
          case ("QUADRUPOLE", "QUADRUPOLE-MOMENT")
            !
            call readi(iref)
            call readj(jref)
            !
            ...
            loop_istate_abquad: do i = 1, iquad
              !
              if (      iref == quadrupoletm(i)%iref &
                  .and. jref == quadrupoletm(i)%jref) then
                  include_state = .true.
                  !
                  iabi_ = sum(iobject(1:Nobjects - 4)) + i
                  !
    ...
    !
    if ((trim(solution_method) == "LOBATTO" .and. grid%nsub /= 6) &
        .or. (trim(solution_method) /= "LOBATTO" .and. grid%nsub == 6)) then
  ...
  subroutine define_quanta_bookkeeping(iverbose, jval, Nestates, Nlambdasigmas)
    ...
    !
    if (iverbose >= 4) then
      write (out, '("The total number sigma/lambda states (size of the sigma-lambda submatrix) = ",i0)') Nlambdasigmas
    endif
    !
...
!                 call splint(field%grid,field%value,spline_wk_vec,field%Nterms,r(i),field%gridvalue(i))
  ...
  subroutine duo_j0(iverbose_, J_list_, enerout, quantaout, nenerout)
    ...
    !
    allocate (vibmat(ngrid, ngrid), vibener(ngrid), contrenergy(ngrid*Nestates), &
              vec(ngrid), contrfunc(ngrid, ngrid*Nestates), stat=alloc)
        ...
        do j = i, Ntotal
          ...
          loop_iso: do iso = 1, Nspinorbits
            ...
            do ipermute = 0, 1
              ...
              if (ipermute == 1 &
                  .and. istate_ == jstate_ &
                  .and. ilambda_we == jlambda_we &
                  .and. nint(sigmai_we - sigmaj_we) == 0 &
                  .and. nint(spini_ - spinj_) == 0) cycle
              !
              ! check if we at the right electronic states
              if (istate /= istate_ .or. jstate /= jstate_) cycle
              !
              ...
              if (ilambda_we - jlambda_we + nint(sigmai_we - sigmaj_we) /= 0 &
                  ...
                  .or. (poten(field%istate)%parity%gu /= 0 &
                        .and. poten(field%istate)%parity%gu /= poten(field%jstate)%parity%gu)) then
                !
              ...
              do isigma2 = -nint(2.0*spini_), nint(2.0*spini_), 2
                ...
                do isigmav = 0, 1
                  ...
                  if ((ilambda_ - jlambda_) /= -nint(sigmai_ - sigmaj_) &
                      .or. abs(sigmai_ - sigmaj_) > 1 &
                      .or. omegai_ /= omegaj_) cycle
                  !
                  ...
                  if (nint(sigmai_ - sigmai) /= 0 &
                      .or. nint(sigmaj_ - sigmaj) /= 0 &
                      .or. ilambda_ /= ilambda &
                      .or. jlambda_ /= jlambda) cycle
                  !
                  f_t = SO*sc
                  !
          ...
          do ild = 1, Nlambdap2q
            ...
            if (lambdap2q(ild)%istate == istate .and. lambdap2q(ild)%jstate == jstate .and. istate == jstate .and. &
                abs(ilambda) == 1 .and. abs(nint(sigmaj - sigmai)) == 1 .and. (ilambda == -jlambda) .and. &
                nint(spini - spinj) == 0 .and. abs(nint(omegai - omegaj)) == 1 .and. &
                nint(sigmaj - sigmai) == nint(omegai - omegaj) .and. &
